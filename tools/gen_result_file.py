...
def parse_args():
  ...
  parser.add_argument('--pkl', dest='pkl_dir',
                      help='pkl dir',
                      default='/home/hezheqi/Project/dev/frame_regression',
                      type=str)
      ...
      for box in boxes[i]:
        ...
        if not poly:
          ...
          res.append('4 ' + x1 + ' ' + y1 + ' ' + x2 + ' ' + y1 + ' ' +
                     x1 + ' ' + y2 + ' ' + x2 + ' ' + y2 + ' ' + str(box[-1])+ '\n')
        else:
          box_str = ' '.join([str(int(b)) for b in box[4:-1]])
          box_str += ' ' + str(box[-1])
...
if __name__ == '__main__':
  ...
  if imdb == 'test':
    ...
    imdb = imdb
      ...
      imdb = imdb
  print(imdb)
  pkl_dir = osp.join(args.pkl_dir, 'output/{}/{}_{}/default/{}_faster_rcnn_iter_{}'
                     .format(args.net, args.type, imdb,  args.net, args.iter))
