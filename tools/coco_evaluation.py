...
def evaluateAttribute(groundtruth, evaluation, resultDict, attributes):
  ...
  for attribute in attributes:
    ...
    res[attribute] = {'attribute': attribute, 'correct': len(correct), 'incorrect': len(incorrect),
...
def evaluateEndToEnd(groundtruth, evaluation, imgIds=None, annIds=[], detection_threshold=0.5):
  ...
  for setting, threshold in zip(['exact', 'distance1'], [0, 1]):
    ...
    res[setting] = {'setting': setting, 'correct': correct, 'incorrect': incorrect, 'ignore': ignore,
                    'accuracy': len(correct) * 1.0 / len(correct + incorrect)}
  return res
...
def printDetailedResults(c_text, detection_results, transcription_results, name):
  ...
  leg_eng_mp = c_text.getAnnIds(imgIds=[], catIds=[('legibility', 'legible'), ('language', 'english'),
                                                   ('class', 'machine printed')], areaRng=[])
  leg_eng_hw = c_text.getAnnIds(imgIds=[],
                                catIds=[('legibility', 'legible'), ('language', 'english'), ('class', 'handwritten')],
                                areaRng=[])
  ...
  if (len(inter(found + n_found, ileg_mp + ileg_hw))) > 0:
    ilto = "%.2f" % (
    100 * len(inter(found, ileg_mp + ileg_hw)) * 1.0 / (len(inter(found + n_found, ileg_mp + ileg_hw))))
  else:
    ilto = 0
  ...
  FN_new = len(inter(n_found, leg_eng_mp + leg_eng_hw)) + len(inter(found, leg_eng_mp + leg_eng_hw)) * (
  1 - transcription_results['exact']['accuracy'])
  t_recall_new = 100 * TP_new / (TP_new + FN_new)
  t_precision_new = 100 * TP_new / (TP_new + FP_new) if (TP_new + FP_new) > 0 else 0
  fscore = "%.2f" % (2 * t_recall_new * t_precision_new / (t_recall_new + t_precision_new)) \
    if (t_recall_new + t_precision_new) > 0 else 0
  ...
  print()
  print(
    "{} & {} & {} & {} & {} & {} & {} & {} & {} & {} & {}".format(lm, lh, ilm, ilh, total, precision, f_score, transAcc,
                                                                  recall_new, precision_new, fscore))
  print()

def my_eval(res_path):
  ct = COCO_Text('/home/hezheqi/data/coco_text/COCO_Text.json')
  our_results = ct.loadRes(res_path)
  our_detections = getDetections(ct, our_results)
  our_endToEnd_results = evaluateEndToEnd(ct, our_results)
  printDetailedResults(ct, our_detections, our_endToEnd_results, 'our approach')

if __name__ == '__main__':
  if len(sys.argv) > 1:
    res_path = sys.argv[1]
  else:
    res_path = '/home/hezheqi/Project/frame_regression/output/res101/' \
               'coco_text_val/default/res101_faster_rcnn_iter_150000/result.json'
  my_eval(res_path)
