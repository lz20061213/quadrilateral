...
class CocoText(imdb):
  ...
  def gt_roidb(self):
    ...
    gt_roidb = [self._load_coco_text_rec_annotation(img_name)
  ...
  def _load_coco_text_rec_annotation(self, img_name):
  ...
  def evaluate_detections(self, all_boxes, output_dir, det_file):
    print("to be update")
    self.write_result(det_file, os.path.join(self._data_path, 'img_list.txt'))

  @classmethod
  def write_result(cls, result_file, img_list_dir):
    out_dir = '/'.join(result_file.strip().split('/')[:-1])
    parsed = []
    results = pickle.load(open(result_file, 'rb'))
    boxes = results[1]
    with open(img_list_dir) as fin:
      for i, name in enumerate(fin):
        image_id = int(name.strip().split('_')[-1])
        for box in boxes[i]:
          score = box[-1]
          if score < 0.7:
            continue
          one_ann = dict()
          one_ann['image_id'] = image_id
          x1, y1, x2, y2 = [int(b) for b in box[:4]]
          bbox = [x1, y1, x2 - x1, y2 - y1]
          one_ann['bbox'] = bbox
          one_ann['utf8_string'] = ''
          one_ann['score'] = str(score)
          parsed.append(one_ann)
    with  open(osp.join(out_dir, 'result.json'), 'w') as fout:
